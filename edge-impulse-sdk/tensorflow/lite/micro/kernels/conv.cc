  ...
  namespace
  {
    ...
    TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node)
    {
      ...
      if (input->type == kTfLiteInt8)
      {
        ...
        conv_params_t conv_params = {
            .in_offset = 0, .out_offset = 0, .stride = {params.stride_width, params.stride_height}, .padding = {data->op_data.padding.width, data->op_data.padding.height}, .dilation = {0, 0}, .activation = {-128, 127}};
        ...
        TfLiteEvalTensor *output)
    {
      ...
      if (dilation_width_factor == 1 && dilation_height_factor == 1)
      {
        ...
        data_dims_t filter_dims = {.width = filter_width, .height = filter_height, 0, 0};
        conv_params_t conv_params = {
            .in_offset = input_offset, .out_offset = output_offset, .stride = {stride_width, stride_height}, .padding = {pad_width, pad_height}, .dilation = {0, 0}, .activation = {activation_min, activation_max}};
